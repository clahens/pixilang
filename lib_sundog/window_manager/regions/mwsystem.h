...
#define GdMalloc(size)                  bmem_new((size))
#define GdCalloc(num,size)              bmem_new((num)*(size))
#define GdRealloc(addr,oldsize,newsize) bmem_resize((addr),(newsize))
#define GdFree(addr)                    bmem_free((addr))
#define GdStrDup(string)                bmem_strdup((string))
