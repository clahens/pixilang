...
int device_start( const char* windowname, int xsize, int ysize, window_manager* wm )
{
    int retval = 0;
    FARPROC proc;
    ...
    utf8_char* vd_str = profile_get_str_value( KEY_VIDEODRIVER, "gapi", 0 );
    wm->vd = VIDEODRIVER_GAPI;
    if( vd_str )
    {
	blog( "Videodriver: %s\n", vd_str );
	if( bmem_strcmp( vd_str, "raw" ) == 0 )
	    wm->vd = VIDEODRIVER_RAW;
	if( bmem_strcmp( vd_str, "gdi" ) == 0 )
	    wm->vd = VIDEODRIVER_GDI;
    }
    else
    {
	blog( "No videodriver selected. Default: gapi\n" );
    }
	...
	if( retval )
	{
	    ...
	    blog( "ERROR: some GX functions not found (%d)\n", retval );
	    return retval;
	}
    }
	...
	if( GXOpenDisplay( hWnd, GX_FULLSCREEN ) == 0 )
	{
	    ...
	    blog( "ERROR: Can't open GAPI display\n" );
	    return 1;
	}
	else
        {
	    ...
	    blog( "GAPI cbxPitch: %d\n", gx_dp.cbxPitch );
	    blog( "GAPI cbyPitch: %d\n", gx_dp.cbyPitch );
	    blog( "GAPI cBPP: %d\n", gx_dp.cBPP );
	    if( gx_dp.ffFormat & kfLandscape ) blog( "kfLandscape\n" );
	    if( gx_dp.ffFormat & kfPalette ) blog( "kfPalette\n" );
	    if( gx_dp.ffFormat & kfDirect ) blog( "kfDirect\n" );
	    if( gx_dp.ffFormat & kfDirect555 ) blog( "kfDirect555\n" );
	    if( gx_dp.ffFormat & kfDirect565 ) blog( "kfDirect565\n" );
	    if( gx_dp.ffFormat & kfDirect888 ) blog( "kfDirect888\n" );
	    if( gx_dp.ffFormat & kfDirect444 ) blog( "kfDirect444\n" );
	    if( gx_dp.ffFormat & kfDirectInverted ) blog( "kfDirectInverted\n" );
	...
	if( rv < 0 )
	{
	    ...
	    blog( "ERROR: Can't open raw framebuffer\n" );
	    return rv;
	}
	blog( "RFB wFormat:\n" );
	if( rfb->wFormat == FORMAT_565 ) blog( "FORMAT_565\n" );
	else if( rfb->wFormat == FORMAT_555 ) blog( "FORMAT_555\n" );
	else 
	{
	    blog( "%d\n", rfb->wFormat );
	    MessageBox( hWnd, L"Can't open raw framebuffer", L"Error", MB_OK );
	...
	wm->wince_zoom_buffer = bmem_new( wm->screen_xsize * wm->screen_ysize * COLORLEN );
	wm->fb_xpitch = 1;
	wm->fb_ypitch = wm->screen_xsize;
    }
...
void device_end( window_manager* wm )
{
    ...
    bmem_free( wm->wince_zoom_buffer );
}
...
uint16 win_key_to_sundog_key( uint16 vk, window_manager* wm )
{
    switch( vk )
    {
    ...
    LPARAM lParam)
{
	...
	case WM_KILLFOCUS:
		...
		if( wm->vd == VIDEODRIVER_GAPI )
		{
    		...
    		wm->gx_suspended = 1;
		send_event( wm->root_win, EVT_SCREENUNFOCUS, 0, 0, 0, 0, 0, 1024, 0, wm );
	    }
	    break;
	case WM_SETFOCUS:
	    if( wm )
		...
		//Redraw all:
		send_event( wm->root_win, EVT_SCREENFOCUS, 0, 0, 0, 0, 0, 1024, 0, wm );
		send_event( wm->root_win, EVT_DRAW, 0, 0, 0, 0, 0, 1024, 0, wm );
	    }
