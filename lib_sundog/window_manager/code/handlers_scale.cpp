...
void ui_scale_reinit( WINDOWPTR win )
{
    ...
    scrollbar_set_parameters( data->ppi, data->cur_ppi, 500, 10, 1, wm );
    ...
    if( data->auto_scale )
    {
	v = wm_get_string( STR_WM_ENABLED_CAP );    
	...
	hide_window( data->ppi, wm );
	hide_window( data->scale, wm );
	hide_window( data->fscale, wm );
    }
    else
    {
	v = wm_get_string( STR_WM_DISABLED_CAP );
	data->ppi->flags &= ~WIN_FLAG_ALWAYS_INVISIBLE;
	data->scale->flags &= ~WIN_FLAG_ALWAYS_INVISIBLE;
	data->fscale->flags &= ~WIN_FLAG_ALWAYS_INVISIBLE;
	show_window( data->ppi, wm );
	show_window( data->scale, wm );
	show_window( data->fscale, wm );
    }

    sprintf( ts, "%s = %s", wm_get_string( STR_WM_AUTO_SCALE ), v );
    button_set_text( data->auto_win, ts, wm );
}
...
int ui_scale_ok_handler( void* user_data, WINDOWPTR win, window_manager* wm )
{
	...
	if( data->auto_scale )
	{
	    profile_remove_key( KEY_PPI, 0 );
	    profile_remove_key( KEY_SCALE, 0 );
	    profile_remove_key( KEY_FONT_SCALE, 0 );
	}
	else
	{
	    ...
	    profile_set_int_value( KEY_PPI, ppi, 0 );
	    profile_set_int_value( KEY_SCALE, scale, 0 );
	    profile_set_int_value( KEY_FONT_SCALE, fscale, 0 );
	}
	profile_save( 0 );
...
int ui_scale_handler( sundog_event* evt, window_manager* wm )
{
	...
	case EVT_AFTERCREATE:
		...
		if( profile_get_int_value( KEY_PPI, 0, 0 ) == 0 && profile_get_int_value( KEY_SCALE, 0, 0 ) == 0 )
		    data->auto_scale = 1;
		else
		    data->auto_scale = 0;
		ui_scale_reinit( win );
